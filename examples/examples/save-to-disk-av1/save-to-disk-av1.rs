...
//! - Receiving AV1 video via WebRTC
//! - Saving AV1 video to IVF file format
//! - Using the AV1 depacketizer for RTP stream processing
...
async fn run(
    ...
    // Everything below is the RTC API! Thanks for using it
    ...
    // Create IVF writer for AV1
    let mut video_writer: Option<IVFWriter<File>> = Some(IVFWriter::new(
        File::create(&video_file)?,
        &IVFFileHeader {
            signature: *b"DKIF",
            version: 0,
            header_size: 32,
            four_cc: *b"AV01", // AV1 FOURCC
            width: 640,
            height: 480,
                ...
                RTCMessage::RtpPacket(track_id, rtp_packet) => {
                    ...
                    if !receiver_id_to_kind.contains_key(&receiver_id) {
                        ...
                        if mime_type == MIME_TYPE_AV1.to_lowercase() {
                            println!("Got AV1 track, saving to disk as {}", video_file);
                        }
                    }

                    // Write video packet
                    if let Some(RtpCodecKind::Video) = receiver_id_to_kind.get(&receiver_id) {
