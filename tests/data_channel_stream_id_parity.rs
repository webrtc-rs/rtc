...
/// Builds a peer with the given answering-DTLS-role configuration.
///
/// `None` leaves it unset, which is how this branch expresses "no preference": unlike the
/// v0.21.x builder, `set_answering_dtls_role` here rejects [`RTCDtlsRole::Auto`] outright, so
/// an unresolved role can only be reached by not configuring one. That rejection is also why
/// this branch cannot emit the non-conformant `a=setup:actpass` answer that RFC 5763 §5
/// forbids — the API refuses the input that would produce it.
fn peer_with_role(answering_dtls_role: Option<RTCDtlsRole>) -> Result<RTCPeerConnection> {
...
struct Outcome {
    ...
    offer_ids: Vec<Option<u16>>,
    answer_ids: Vec<Option<u16>>,
...
async fn connect(
    offer_cfg: Option<RTCDtlsRole>,
    answer_cfg: Option<RTCDtlsRole>,
) -> Result<Outcome> {
...
async fn stream_id_parity_matches_negotiated_dtls_role_across_all_role_configurations() -> Result<()>
{
    ...
    use RTCDtlsRole::{Client, Server};

    // `None` is this branch's spelling of "no preference"; see `peer_with_role`.
    let cases = [
        (
            "1. offer=Client   answer=Server ",
            Some(Client),
            Some(Server),
        ),
        (
            "2. offer=Server   answer=Client ",
            Some(Server),
            Some(Client),
        ),
        ("3. offer=Client   answer=default", Some(Client), None),
        ("4. offer=default  answer=Server ", None, Some(Server)),
        ("5. offer=Server   answer=default", Some(Server), None),
        ("6. offer=default  answer=Client ", None, Some(Client)),
        ("7. offer=default  answer=default", None, None),
    ];
...
async fn colliding_stream_ids_prevent_both_peers_channels_from_opening() -> Result<()> {
    ...
    let out = connect(None, Some(RTCDtlsRole::Server)).await?;
