...
/// Fallback when a peer reports no deadline of its own.
const DEFAULT_TIMEOUT_DURATION: Duration = Duration::from_secs(30);

fn peer(setting_engine: SettingEngine) -> Result<RTCPeerConnection> {
...
async fn large_message_is_delivered_when_it_exceeds_the_receive_buffer() -> Result<()> {
    ...
    offer_setting_engine.set_sctp_max_message_size(SctpMaxMessageSize::Bounded(
    ...
    answer_setting_engine.set_sctp_max_message_size(SctpMaxMessageSize::Bounded(
        SctpMaxMessageSize::MAX_MESSAGE_SIZE,
    ));
    answer_setting_engine.set_sctp_max_receive_buffer_size(RECV_BUFFER);

    let mut offer_pc = peer(offer_setting_engine)?;
    let mut answer_pc = peer(answer_setting_engine)?;
