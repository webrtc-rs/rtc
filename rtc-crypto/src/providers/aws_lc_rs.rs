...
pub struct AwsLcRsProvider {
    crypto: AwsLcRsCrypto,
    random: AwsLcRsRandom,
}

impl AwsLcRsProvider {
    /// Creates an AWS-LC-RS provider.
    #[must_use]
...
impl RTCCryptoProvider for AwsLcRsProvider {
...
/// AWS-LC-RS-backed operations, with RustCrypto fallbacks for primitives that are not exposed by
/// its stable high-level API: MD5, AES block/CTR/CBC, and AES-CCM.
#[derive(Default)]
pub struct AwsLcRsCrypto;

/// AWS-LC-RS's operating-system-backed secure random source.
#[derive(Default)]
pub struct AwsLcRsRandom;
...
impl RTCCrypto for AwsLcRsCrypto {
    ...
    fn new_aead(
        ...
        match algorithm {
            ...
            AeadAlgorithm::Aes128Gcm => AwsLcRsAead::create(&aead::AES_128_GCM, key),
            AeadAlgorithm::Aes256Gcm => AwsLcRsAead::create(&aead::AES_256_GCM, key),
            AeadAlgorithm::ChaCha20Poly1305 => AwsLcRsAead::create(&aead::CHACHA20_POLY1305, key),
        }
    }
    ...
    fn start_key_exchange(
        ...
        AwsLcRsKeyExchange::start(algorithm)
    }
    ...
    fn generate_signing_key(
        ...
        AwsLcRsSigningKey::generate(scheme)
    }
    ...
    fn import_signing_key(
        ...
        AwsLcRsSigningKey::import(scheme, pkcs8_der)
    }
...
impl AeadCipher for AwsLcRsAead {
    ...
    fn open_in_place(
        ...
        self.key
            .open_in_place_separate_tag(nonce, aead::Aad::from(aad), tag, ciphertext_and_plaintext)
            .map(|_| ())
            .map_err(|_| CryptoError::AuthenticationFailed)
    }
}

struct AwsLcRsKeyExchange {
    algorithm: KeyExchangeAlgorithm,
...
impl AwsLcRsKeyExchange {
    fn start(algorithm: KeyExchangeAlgorithm) -> Result<Box<dyn ActiveKeyExchange>, CryptoError> {
        ...
        let private_key = agreement::PrivateKey::generate(backend_algorithm)
...
impl ActiveKeyExchange for AwsLcRsKeyExchange {
    ...
    fn complete(self: Box<Self>, peer_public_key: &[u8]) -> Result<SecretVec, CryptoError> {
        ...
        agreement::agree(
            ...
            |secret| Ok(SecretVec::new(secret.to_vec())),
        )
    }
}

enum AwsLcRsSigningKeyKind {
    Ed25519(signature::Ed25519KeyPair),
    EcdsaP256(signature::EcdsaKeyPair),
    Rsa(signature::RsaKeyPair),
}

struct AwsLcRsSigningKey {
    scheme: SignatureScheme,
    kind: AwsLcRsSigningKeyKind,
    public_key: Vec<u8>,
    public_key_encoding: PublicKeyEncoding,
    pkcs8_der: SecretVec,
}

impl AwsLcRsSigningKey {
    ...
    fn import(
        ...
        let (kind, public_key, public_key_encoding) = match scheme {
            SignatureScheme::Ed25519 => {
                ...
                (
                    AwsLcRsSigningKeyKind::Ed25519(key),
                    public,
            ...
            SignatureScheme::EcdsaP256Sha256 => {
                ...
                let public = key.public_key().as_ref().to_vec();
                (
                    AwsLcRsSigningKeyKind::EcdsaP256(key),
                    public,
...
impl SigningKey for AwsLcRsSigningKey {
    ...
    fn sign(&self, scheme: SignatureScheme, message: &[u8]) -> Result<Vec<u8>, CryptoError> {
        ...
        match &self.kind {
            AwsLcRsSigningKeyKind::Ed25519(key) => Ok(key.sign(message).as_ref().to_vec()),
            AwsLcRsSigningKeyKind::EcdsaP256(key) => key
