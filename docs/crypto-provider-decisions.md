...
The pre-migration workspace intentionally rejects both built-ins together and cannot yet build the full RTC stack without one. Baseline CI names those two future configurations but does not pretend they pass. P1-09 replaces the placeholders with passing both-provider and no-built-in/custom-provider jobs.
